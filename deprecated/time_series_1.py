...
if __name__ == "__main__":
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(20, n_rows * (500 / 96)), constrained_layout=True)  # 500 pixels height per chart
    axes = axes.flatten()
    ...
    for name, func in augmentations.items():
        try:
            ...
            axes[valid_idx].plot(time_axis, original_feature, label="Original", alpha=0.3)
            axes[valid_idx].plot(time_axis, feature, label=name.capitalize(), alpha=0.6)
            axes[valid_idx].fill_between(time_axis, original_feature, feature, color='#2ca02c', alpha=0.08, label='Deviation')
    ...
    plt.savefig("plots/all_augmentations_combined.png")
    plt.show()
